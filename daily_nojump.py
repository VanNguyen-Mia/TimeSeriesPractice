...
## READ THE DATA 
df = pd.read_csv('./art_daily_nojump.csv')
...
result.plot()
plt.suptitle('Seasonal Decomposition of art-daily-nojump')
plt.tight_layout()
plt.show()
...
plt.title('Seasonal Component of art-daily-nojump')
plt.xlabel('Day')
...
plt.title('art_daily_nojump with and without Seasonal Component')
plt.xlabel('Day')
...
print("Cleaned data with z-scores:", cleaned_df1)
...
print("Data after removing outliers with boxplots:")
print(cleaned_df2)

################################ TASK 3: FORECAST TIME SERIES USING ARIMA ########################################
# Check for stationarity:
result = adfuller(cleaned_df1["value"])
print(f"ADF Statistic: {result[0]}")
print(f"p-value: {result[1]}")

# Fill missing values
cleaned_df1.fillna(method='ffill', inplace=True)

# Perform differencing:
if result[1] > 0.05:  
    cleaned_df1["value"] = cleaned_df1["value"].diff().dropna()
    result = adfuller(cleaned_df1["value"])
...
# Finding lags:
pd.plotting.autocorrelation_plot(cleaned_df1)
...
plot_acf(cleaned_df1['value'], lags = 500)
plt.title('ACF')
plt.show() # Result shows q = 1


plot_pacf(cleaned_df1['value'], lags = 500)
plt.title('PACF')
